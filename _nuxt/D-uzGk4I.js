import{a as n}from"./DbNIgRzX.js";import{d as t,c,o as a}from"./D1LHy2jC.js";import"./anvSD_Ip.js";const k=t({__name:"a3b16d10",setup(s){const e={lang:"ja-JP",title:"プロジェクト構造の規約",html:`<div class="kecare"><h1 id="プロジェクト構造と規約kecare">プロジェクト構造と規約（Kecare）</h1>
<p>どんな技術スタックやフレームワークでも、VueでもReactでもSvelteでも、従来のサーバーサイドレンダリングフレームワークでも、
どんな古いプロジェクトでも、あるいは自分が好きな技術スタックで作成した新しいプロジェクトでも、.kecareディレクトリを新規作成するだけで、ブログに変身します。
定義したルールに基づいて、ブログページを生成します。</p>
<hr>
<h2 id="1-ディレクトリ構造">1. ディレクトリ構造</h2>
...
<p><strong>.kecare/</strong> は Kecare システムの中で最も重要な部分です：<strong>執筆者のコンテンツはここに置き、メニューもここに置き、テーマ作者もデフォルトでここからコンテンツを取得します</strong>、にぇ～</p>
<h2 id="2-kecare-ディレクトリの規約">2. <strong>.kecare/</strong> ディレクトリの規約</h2>
<h3 id="21-kecarearticles記事ディレクトリ">2.1 <strong>.kecare/articles/</strong>：記事ディレクトリ</h3>
<ul>
<li>位置固定：<strong>テーマディレクトリ/.kecare/articles/</strong></li>
<li>ファイルタイプ：現在は <strong><code>*.md</code></strong> のみサポート</li>
<li>推奨命名：安定した、読みやすい英語のファイル名を使用（例：<strong><code>getting-started.md</code></strong>）、頻繁な名前変更は避けてください</li>
</ul>
<p>なぜファイル名をむやみに変更しない方が良いのですか？</p>
<p>Kecareの一般的な実装では、記事の <strong>id</strong> はファイル名のハッシュから生成されることが多く、それによって安定したリンクが形成されます</p>
<p>ファイル名が変わった → <strong>id</strong> が変わる可能性あり → 古いリンクが無効になる可能性あり、にゃん~</p>
<h3 id="22-kecaremenusメニューディレクトリ">2.2 <strong>.kecare/menus/</strong>：メニューディレクトリ</h3>
<ul>
<li>位置固定：<strong>テーマディレクトリ/.kecare/menus/</strong></li>
<li>ファイル名固定：<strong><code>*.menu.source.ts</code></strong></li>
<li>ファイル内容の約束：<strong>navItems: NavItem[]</strong> をエクスポートする</li>
</ul>
<h2 id="3-テーマ層でのデータの消費方法テーマ作者向け">3. テーマ層でのデータの消費方法（テーマ作者向け）</h2>
<p>Kecareのジェネレータは通常、記事を構造化されたオブジェクト配列（例えば <strong><code>articles: Article[]</code></strong>）に解析します。</p>
